      ...
      <div class="col-lg-8" style="margin: 0px auto!important;width: 100%!important;">
        <div class="row">
          ...
          <table class="table table-striped">
            ...
            <tbody>
              <?php
                $reponse = $db -> query('SELECT COUNT(ordinateur.id) AS nombre FROM ordinateur WHERE ordinateur.category_id = 1 AND user_id IS NULL');
                $countSql = $reponse -> fetch();
                $reponse = $db -> query('SELECT id , UPPER(code) as code, UPPER(marque) as marque,CONCAT(dernier_utilisateur, " ", DATE_FORMAT(ordinateur.date_affectation,"%d/%m/%Y %H:%i:%s")) AS date_affectation, UPPER(observation) as observation FROM ordinateur WHERE category_id = 1 AND user_id IS NULL');
